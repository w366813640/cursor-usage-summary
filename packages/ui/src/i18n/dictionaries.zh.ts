...
/**
 * Chinese (Simplified) dictionary — lazy-loaded chunk (perf plan 4.3).
 *
 * Loaded on demand by I18nProvider via `loadDictionary('zh')` so the
 * default-locale bundle doesn't carry both languages. Keys mirror
 * dictionaries.ts (en); missing keys fall back to English at lookup.
 */
